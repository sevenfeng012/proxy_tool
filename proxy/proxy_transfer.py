...
class ProxyHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if isAjmdStatReq!=-1:
            ...
            targeturl = self.request.uri[len(standard):]

            showUrl = targeturl

            targeturl = urllib.quote(targeturl)

            fetchUrl = 'http://121.40.161.184:81/abtest_format_check_interface.php?mode=online&request='+targeturl

            s = requests.session()
            s.keep_alive = False

            ret = requests.get(fetchUrl)

            print fetchUrl

            host = socket.gethostbyname(socket.gethostname())

            # msg = '\n\n\n[******]'+(ret.text).replace('\'','') +'[******]'+urllib.unquote(showUrl)
            msg = '\n\n\n[******]'+(ret.text).replace('\'','') +'[******]'+urllib.unquote(showUrl)
            print "\n-----" + msg
            cmdStr = "mqtt pub -t 'stat' -h %s -m \' %s \'" %(host,msg)
            print "\n cmdStr " + cmdStr
